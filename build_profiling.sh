...
sed -i -e '/--virtual/! s/apk add\ --no-cache/DEBIAN_FRONTEND=noninteractive apt-get\ install\ --no-install-recommends -y build-essential libjansson-dev/g' "Dockerfile.profiling"
sed -i -e '/apk add.*\ --virtual/d' "Dockerfile.profiling"
sed -i -e '/apk\ del/d' "Dockerfile.profiling"
...
sed -i -e 's/openssl-dev/libssl1.0-dev/g' "Dockerfile.profiling"
